...
RESPONSEDEFAULTFINDSTREAMS_ANY_OF_SCHEMAS = ["PublicPageAnnotatedULIDUlidPydanticAnnotation", "PublicPagePublicStreamDatasource"]

class ResponseDefaultFindStreams(BaseModel):
    """
    ResponseDefaultFindStreams
    """

    # data type: PublicPagePublicStreamDatasource
    anyof_schema_1_validator: Optional[PublicPagePublicStreamDatasource] = None
    ...
    if TYPE_CHECKING:
        actual_instance: Optional[Union[PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicStreamDatasource]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "PublicPageAnnotatedULIDUlidPydanticAnnotation", "PublicPagePublicStreamDatasource" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ResponseDefaultFindStreams.model_construct()
        error_messages = []
        # validate data type: PublicPagePublicStreamDatasource
        if not isinstance(v, PublicPagePublicStreamDatasource):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PublicPagePublicStreamDatasource`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ResponseDefaultFindStreams with anyOf schemas: PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicStreamDatasource. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[PublicPagePublicStreamDatasource] = None
        try:
            instance.actual_instance = PublicPagePublicStreamDatasource.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ResponseDefaultFindStreams with anyOf schemas: PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicStreamDatasource. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicStreamDatasource]]:
