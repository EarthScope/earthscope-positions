...
RESPONSEDEFAULTFINDNETWORKS_ANY_OF_SCHEMAS = ["PublicPageAnnotatedULIDUlidPydanticAnnotation", "PublicPagePublicNetworkDatasource"]

class ResponseDefaultFindNetworks(BaseModel):
    """
    ResponseDefaultFindNetworks
    """

    # data type: PublicPagePublicNetworkDatasource
    anyof_schema_1_validator: Optional[PublicPagePublicNetworkDatasource] = None
    ...
    if TYPE_CHECKING:
        actual_instance: Optional[Union[PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicNetworkDatasource]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "PublicPageAnnotatedULIDUlidPydanticAnnotation", "PublicPagePublicNetworkDatasource" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ResponseDefaultFindNetworks.model_construct()
        error_messages = []
        # validate data type: PublicPagePublicNetworkDatasource
        if not isinstance(v, PublicPagePublicNetworkDatasource):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PublicPagePublicNetworkDatasource`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ResponseDefaultFindNetworks with anyOf schemas: PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicNetworkDatasource. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[PublicPagePublicNetworkDatasource] = None
        try:
            instance.actual_instance = PublicPagePublicNetworkDatasource.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ResponseDefaultFindNetworks with anyOf schemas: PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicNetworkDatasource. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicNetworkDatasource]]:
