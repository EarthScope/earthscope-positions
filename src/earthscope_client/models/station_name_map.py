...
class StationNameMap(BaseModel):
    """
    Contains names for a Station represented as a mapping from `namespace -> name`.  - GNSS stations always have a 9-character name with namespace `IGS`. They additionally have a 4-character name with namespace `4CHARID` when the `IGS` name is for monument/receiver `00`.  - Borehole strainmeter stations always have a name with namespace `FDSN`. They may additionally contain a name with namespace `4CHARID`.  - Seismic stations normally have a single name in namespace `FDSN` which is the [FDSN Source Identifier](https://docs.fdsn.org/projects/source-identifiers/en/v1.0/index.html).
    """ # noqa: E501
    fdsn: Optional[StrictStr] = Field(default=None, alias="FDSN")
    igs: Optional[StrictStr] = Field(default=None, alias="IGS")
    var_4_charid: Optional[StrictStr] = Field(default=None, alias="4CHARID")
    __properties: ClassVar[List[str]] = ["FDSN", "IGS", "4CHARID"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of StationNameMap from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of StationNameMap from a dict"""
        if obj is None:
            return None
