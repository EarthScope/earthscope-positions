...
class Item(BaseModel):
    """
    Class representing a dropoff item in the API response. This is a more limited view than AdminItem, which is used for internal/admin purposes.  Attributes:     path (str): The S3 object key of the dropoff file.     created_at (AwareDatetime): The timestamp when the dropoff was received, serialized as \"received_at\" in the API response.     size (int): The size of the dropoff file in bytes.     sha256 (str): The SHA-256 hash of the dropoff file, serialized as \"hash\" in the API response.     status (str): The current status of the dropoff (e.g., \"pending\", \"processed\", \"error\").     status_message (str | None): An optional message providing additional details about the status.     structured_messages (List[str]): A list of structured messages associated with the dropoff.     structured_messages_were_truncated (bool): Indicates whether the structured_messages list was truncated due to length constraints.     execution_id (str | None): An optional identifier for the processing execution associated with this dropoff.
    """ # noqa: E501
    ...
    structured_messages: Optional[List[StrictStr]] = None
    structured_messages_were_truncated: Optional[StrictBool] = False
    execution_id: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["path", "received_at", "size", "hash", "status", "status_message", "structured_messages", "structured_messages_were_truncated", "execution_id"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "structured_messages": obj.get("structured_messages"),
            "structured_messages_were_truncated": obj.get("structured_messages_were_truncated") if obj.get("structured_messages_were_truncated") is not None else False,
