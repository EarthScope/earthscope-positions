...
RESPONSEDEFAULTFINDSESSIONS_ANY_OF_SCHEMAS = ["PublicPageAnnotatedULIDUlidPydanticAnnotation", "PublicPagePublicSessionDatasource"]

class ResponseDefaultFindSessions(BaseModel):
    """
    ResponseDefaultFindSessions
    """

    # data type: PublicPagePublicSessionDatasource
    anyof_schema_1_validator: Optional[PublicPagePublicSessionDatasource] = None
    # data type: PublicPageAnnotatedULIDUlidPydanticAnnotation
    anyof_schema_2_validator: Optional[PublicPageAnnotatedULIDUlidPydanticAnnotation] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicSessionDatasource]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "PublicPageAnnotatedULIDUlidPydanticAnnotation", "PublicPagePublicSessionDatasource" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ResponseDefaultFindSessions.model_construct()
        error_messages = []
        # validate data type: PublicPagePublicSessionDatasource
        if not isinstance(v, PublicPagePublicSessionDatasource):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PublicPagePublicSessionDatasource`")
        else:
            return v

        # validate data type: PublicPageAnnotatedULIDUlidPydanticAnnotation
        if not isinstance(v, PublicPageAnnotatedULIDUlidPydanticAnnotation):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PublicPageAnnotatedULIDUlidPydanticAnnotation`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ResponseDefaultFindSessions with anyOf schemas: PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicSessionDatasource. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[PublicPagePublicSessionDatasource] = None
        try:
            instance.actual_instance = PublicPagePublicSessionDatasource.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[PublicPageAnnotatedULIDUlidPydanticAnnotation] = None
        try:
            instance.actual_instance = PublicPageAnnotatedULIDUlidPydanticAnnotation.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ResponseDefaultFindSessions with anyOf schemas: PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicSessionDatasource. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicSessionDatasource]]:
