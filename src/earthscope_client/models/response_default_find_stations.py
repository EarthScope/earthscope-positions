...
RESPONSEDEFAULTFINDSTATIONS_ANY_OF_SCHEMAS = ["PublicPageAnnotatedULIDUlidPydanticAnnotation", "PublicPagePublicStationDatasource"]

class ResponseDefaultFindStations(BaseModel):
    """
    ResponseDefaultFindStations
    """

    # data type: PublicPagePublicStationDatasource
    anyof_schema_1_validator: Optional[PublicPagePublicStationDatasource] = None
    ...
    if TYPE_CHECKING:
        actual_instance: Optional[Union[PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicStationDatasource]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "PublicPageAnnotatedULIDUlidPydanticAnnotation", "PublicPagePublicStationDatasource" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ResponseDefaultFindStations.model_construct()
        error_messages = []
        # validate data type: PublicPagePublicStationDatasource
        if not isinstance(v, PublicPagePublicStationDatasource):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PublicPagePublicStationDatasource`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ResponseDefaultFindStations with anyOf schemas: PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicStationDatasource. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[PublicPagePublicStationDatasource] = None
        try:
            instance.actual_instance = PublicPagePublicStationDatasource.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ResponseDefaultFindStations with anyOf schemas: PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicStationDatasource. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], PublicPageAnnotatedULIDUlidPydanticAnnotation, PublicPagePublicStationDatasource]]:
