...
class StreamNameMap(BaseModel):
    """
    Contains names for a Stream represented as a mapping from `namespace -> name`.  - Streams should always have a descriptive name in namespace `SN`.  - Streams used in ShakeAlert additionally have a name in namespace `GEOSNCL`.
    """ # noqa: E501
    geosncl: Optional[StrictStr] = Field(default=None, alias="GEOSNCL")
    sn: Optional[StrictStr] = Field(default=None, alias="SN")
    __properties: ClassVar[List[str]] = ["GEOSNCL", "SN"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of StreamNameMap from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of StreamNameMap from a dict"""
        if obj is None:
            return None
